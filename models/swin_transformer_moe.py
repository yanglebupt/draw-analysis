...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        q, k, v = self.q_proj(x).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 3, 1, 4), \
                  self.k_proj(x).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 3, 1, 4), \
                  self.v_proj(x).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 3, 1, 4)
